...
#define CHESS_TASK_STACK_SIZE (5 * 1024)
#define CHESS_TASK_PRIORITY (configMAX_PRIORITIES - 1)
...
int main()
{
    ...
    xTaskCreate(mqtt_client_task, "MQTT Client task", MQTT_CLIENT_TASK_STACK_SIZE,
                NULL, MQTT_CLIENT_TASK_PRIORITY, &MQTT_Client_handle);
    xTaskCreate(chess_task, "Chess task", CHESS_TASK_STACK_SIZE,
                NULL, CHESS_TASK_PRIORITY, &Chess_Task);
