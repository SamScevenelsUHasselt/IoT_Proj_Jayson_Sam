...
void subscriber_task(void *pvParameters)
{
    ...
    subscriber_msg_q = xQueueCreate(1u, sizeof(chess_message_t));
...
void mqtt_subscription_callback(cy_mqtt_publish_info_t *received_msg_info)
{
    ...
    if ((strlen("SamReady") == received_msg_len) && (strncmp("SamReady", received_msg, received_msg_len) == 0))
    {
    	subscriber_q_data.cmd = ready;
    }

    else if ((strlen("sam you white") == received_msg_len) &&
    		(strncmp("sam you white", received_msg, received_msg_len) == 0))
    {
    	subscriber_q_data.cmd = you_are_white;
    }
    else if ((strlen("sam you black") == received_msg_len) &&
    		(strncmp("sam you black", received_msg, received_msg_len) == 0))
    {
    	subscriber_q_data.cmd = you_are_black;
    }

    else if(received_msg[0] == 'W' || received_msg[0] == 'B'){ //msg will look like: "W1-2 to 4-8"
    	subscriber_q_data.cmd = move;
    	subscriber_q_data.from_x = received_msg[1] - '0';
    	subscriber_q_data.from_y = received_msg[3] - '0';
    	subscriber_q_data.to_x = received_msg[8] - '0';
    	subscriber_q_data.to_y = received_msg[10] - '0';
    	if(received_msg[0] == 'W'){subscriber_q_data.color = White;}
    	else{subscriber_q_data.color = Black;}
    	printf("%d %d %d %d\n\n",subscriber_q_data.to_x,subscriber_q_data.to_y,subscriber_q_data.from_x,subscriber_q_data.from_y);
    }

    print_heap_usage("MQTT subscription callback");

    /* Send the command and data to subscriber task queue */
    xQueueSend(subscriber_msg_q, &subscriber_q_data, portMAX_DELAY);
}
