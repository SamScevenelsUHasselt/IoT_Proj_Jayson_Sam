...
void mqtt_subscription_callback(cy_mqtt_publish_info_t *received_msg_info)
{
    ...
    if ((strlen("JaysonReady") == received_msg_len) &&
		(strncmp("JaysonReady", received_msg, received_msg_len) == 0))
	{
		subscriber_q_data.cmd = ready;
	}else if ((strlen("json you white") == received_msg_len) &&
			(strncmp("json you white", received_msg, received_msg_len) == 0))
	{
		subscriber_q_data.cmd = you_are_white;
	}
	else if ((strlen("json you black") == received_msg_len) &&
			(strncmp("json you black", received_msg, received_msg_len) == 0))
	{
    ...
    xQueueOverwrite(subscriber_msg_q, &subscriber_q_data);
}
